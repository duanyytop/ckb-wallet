...
public class Bip32ECKeyPair extends ECKeyPair {
    ...
    public static Bip32ECKeyPair create(BigInteger privateKey, byte[] chainCode) {
        return new Bip32ECKeyPair(
                privateKey, publicKeyFromPrivate(privateKey), 0, chainCode, null);
    }
    ...
    private Bip32ECKeyPair deriveChildKey(int childNumber) {
        if (!hasPrivateKey()) {
            ...
            Arrays.fill(il, (byte) 0);
            ECPoint ki = publicPointFromPrivate(ilInt).add(getPublicKeyPoint());

            return new Bip32ECKeyPair(
                    null, publicFromPoint(ki.getEncoded(true)), childNumber, chainCode, this);
        } else {
            ByteBuffer data = ByteBuffer.allocate(37);
    ...
    public ECPoint getPublicKeyPoint() {
        if (publicKeyPoint == null) {
            publicKeyPoint = publicPointFromPrivate(getPrivateKey());
        }
    ...
    private static BigInteger publicFromPoint(byte[] bits) {
